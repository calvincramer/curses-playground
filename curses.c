...
void basics() {
  ...
  mvprintw(2, 0, "The window size is %d rows by %d columns", win_max_y,
           win_max_x);
  ...
  if (has_colors() == true) {
    ...
    mvprintw(4, 1, "The color pair 1 has foreground %d and background %d",
...
void mouse() {
  ...
  mask =
      BUTTON1_PRESSED | BUTTON_SHIFT | BUTTON2_CLICKED | REPORT_MOUSE_POSITION;
  mousemask(mask, &oldmask);  // Assigns new mask and saves previous one
  mouseinterval(0);           // No delay for mouse clicks

  mvprintw(
      0, 0,
      "Click the left mouse button in and outside the window. q to go next");
  ...
  while (true) {
    ...
    clrtoeol();
    if (wmouse_trafo(win, &wy, &wx, FALSE) ==
        TRUE) {  // Convert screen -> window coordinates
      mvprintw(12, 1,
               "The window coordinates are %d, %d and the screen "
               "coordinates are %d, %d",
               wy, wx, event.y, event.x);
      sx = wx;
      sy = wy;
      wmouse_trafo(win, &sy, &sx,
                   TRUE);  // Convert screen -> window coordinates
      mvprintw(13, 1, "The window coordinates are %d, %d(converted back)", sy,
               sx);
      clrtoeol();
    } else {
      mvprintw(12, 1,
               "The screen coordinates are %d, %d and not in the window.", wy,
               wx);
      clrtoeol();
    }
    wrefresh(win);
    ...
    if (event.bstate & BUTTON1_PRESSED) {
      mvwprintw(win, 8, 1,
                "You pressed the left mouse button at %d, %d [in window? %d]",
                event.x, event.y, inwindow);
      if (inwindow == TRUE) {
        ...
        mvaddch(event.y, event.x, ACS_CKBOARD | A_DIM);
        move(event.y, event.x);
        refresh();
      }
    }
    if (event.bstate & BUTTON2_CLICKED) {
      mvwprintw(win, 9, 1, "You clicked the right mouse button at %d, %d",
                event.x, event.y);
      clrtoeol();
      wrefresh(win);
    }
